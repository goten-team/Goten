...
void AesGcm::decrypt(const unsigned char* ciphertext, size_t ciphertext_len, unsigned char* plaintext, size_t plaintext_len, unsigned char* blind) {
	assert(plaintext_len==ciphertext_len);
	(void)ciphertext_len;
	...
	m_len+=plaintext_len;
	intel_aes_gcmDEC(ciphertext,plaintext,&gctx,plaintext_len, blind);
}
